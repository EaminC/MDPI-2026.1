...
\raggedright
\texttt{\textbf{Input:} user set $U$, item set $I$, rating matrix $R$, rank $k$, learning rate $\eta$, regularization $\lambda$, epochs num\_epochs}\\
\texttt{\textbf{Output:} predicted rating matrix $\hat{R}$}\\
\texttt{\textbf{Initialize} $U_k \in \mathbb{R}^{m \times k}$ and $V_k \in \mathbb{R}^{n \times k}$ randomly}\\
\texttt{\textbf{for} epoch = 1 to num\_epochs \textbf{do}}\\
\texttt{\quad \textbf{for} each observed $(u,i)$ in $R$ \textbf{do}}\\
\texttt{\quad\quad \textbf{compute} error $e_{ui} = R_{ui} - (U_k \Sigma_k V_k^T)_{ui}$}\\
\texttt{\quad\quad \textbf{update} $U_k[u,:] \leftarrow U_k[u,:] + \eta (e_{ui} V_k[i,:] - \lambda U_k[u,:])$}\\
\texttt{\quad\quad \textbf{update} $V_k[i,:] \leftarrow V_k[i,:] + \eta (e_{ui} U_k[u,:] - \lambda V_k[i,:])$}\\
\texttt{\quad \textbf{end for}}\\
\texttt{\textbf{end for}}\\
\texttt{\textbf{Compute} $\hat{R} = U_k \Sigma_k V_k^T$}\\
\texttt{\textbf{return} $\hat{R}$}\\
\rule{\columnwidth}{1pt}
\end{algorithm}
...
\rule{\columnwidth}{1pt}
\raggedright
\texttt{\textbf{Input:} user preference vector $\mathbf{p}_u$, item vectors $\{\mathbf{v}_i\}$, top-$N$}\\
\texttt{\textbf{Output:} ranked recommendation list}\\
\texttt{\textbf{for} each candidate item $i$ \textbf{do}}\\
\texttt{\quad \textbf{compute} similarity $\text{sim}(u,i) = \mathbf{p}_u \cdot \mathbf{v}_i$}\\
\texttt{\textbf{end for}}\\
\texttt{\textbf{Sort} candidates by similarity in descending order}\\
\texttt{\textbf{return} top-$N$ items}\\
\rule{\columnwidth}{1pt}
\end{algorithm}
