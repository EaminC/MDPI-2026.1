...
\subsection{Overview}
In recent years, large language model (LLM) technologies~\cite{wang2022pre} have advanced rapidly and have addressed a range of long-standing open problems in natural language processing. In this work, we primarily adopt GPT-3.5-turbo~\cite{brown2020language} as a pretrained LLM to provide an interface for semantic understanding. Leveraging their extensive pretrained knowledge and strong semantic modeling capabilities, LLMs offer powerful functional primitives for recommender systems, including semantic feature extraction, workflow understanding, and sentiment analysis. As a related example beyond recommendation, prior work explored emotion-aware scene adaptation under efficiency (bandwidth) constraints for generating animated shorts~\cite{yang2024emotion}. These capabilities form the foundation of the hybrid recommender system framework proposed in this work.
...
\begin{figure}[t]
    \centering
    \IfFileExists{figures/part1.pdf}{%
        \includegraphics[width=0.95\linewidth]{figures/part1.pdf}%
    }{%
        \fbox{Missing figure: figures/part1.pdf}%
    }
    \caption{Stage 1: Agent memory module (Sew stage) that integrates long-/short-term preferences and static/dynamic user information.}
    \label{fig:agent-flow-1}
\end{figure}

\begin{figure}[t]
    \centering
    \IfFileExists{figures/part2.pdf}{%
        \includegraphics[width=0.95\linewidth]{figures/part2.pdf}%
    }{%
        \fbox{Missing figure: figures/part2.pdf}%
    }
    \caption{Stage 2: Hybrid candidate recall and pre-ranking to improve recall quality while maintaining result diversity.}
    \label{fig:agent-flow-2}
\end{figure}

\begin{figure}[t]
    \centering
    \IfFileExists{figures/part3.pdf}{%
        \includegraphics[width=0.95\linewidth]{figures/part3.pdf}%
    }{%
        \fbox{Missing figure: figures/part3.pdf}%
    }
    \caption{Stage 3: Human feedback and propagation back to the database for iterative updates and refinement.}
    \label{fig:agent-flow-3}
\end{figure}

\begin{listing}[t]
\caption{Example payloads for the numbered steps (1--6) in Figures~\ref{fig:agent-flow-1}--\ref{fig:agent-flow-3} (JSON-like representation)}
\label{lst:agent-flow-payloads}
\rule{\columnwidth}{1pt}
\raggedright
\textbf{(1) User request (dynamic memory / query)}
\begin{lstlisting}[style=jsonlike]
{
  "type": "meal",
  "classification": "sichuan food",
  "name": "chuanweiju",
  "filter": ["distance", 0]
}
\end{lstlisting}

\textbf{(2) Preference summary (static memory / persona)}
\begin{lstlisting}[style=jsonlike]
{
  "description": "He is such a ...",
  "preferred_class": "sichuan food",
  "filter": ["distance", 0]
}
\end{lstlisting}

\textbf{(3) Feedback-adjusted constraints (after user feedback)}
\begin{lstlisting}[style=jsonlike]
{
  "type": "meal",
  "classification": "sichuan food",
  "name": "chuanweiju",
  "filter": ["distance", 0],
  "modified_filter": ["comment", 1]
}
\end{lstlisting}

\textbf{(4/5/6) Candidate item list (shared payload)}
\begin{lstlisting}[style=jsonlike]
{
...
Consider an SVD-based interaction recommender. To emphasize recent behaviors, we introduce multiple weight modulation layers that reweight interactions by recency. To avoid confusion with the item index \(i\) used in \(R_{ui}\), we use \(t\in\{1,\ldots,T\}\) to index the position of an interaction in a user's time-ordered history (from oldest to most recent), and let \(T\) denote the total length (time horizon) of that history. A simple linear recency modulation can be written as:
\begin{equation}
w_t^{(n)}=\frac{t}{T}.
\end{equation}

More generally, we use a smooth activation (sigmoid) to control the attention range at different time scales. Let \(j_n\in\{1,\ldots,T\}\) denote the high-attention center index at time scale \(n\), and define the normalized center as \(c_n = j_n/T\in(0,1]\). Let \(\sigma(x)=\frac{1}{1+\exp(-x)}\). Then:
\begin{equation}
w_t^{(n)}=\sigma\!\left(\alpha_n\left(\frac{t}{T}-c_n\right)\right)
= \frac{1}{1+\exp\!\left(-\alpha_n\left(\frac{t}{T}-c_n\right)\right)} ,
\end{equation}
...
\texttt{\textbf{end for}}\\
\texttt{\textbf{return} $\{\hat{\mathcal{R}}^n\}$}\\
\rule{\columnwidth}{1pt}
\end{algorithm}
...
In addition, LLM-based reasoning can exhibit hallucinations or inconsistent judgments in edge cases; multi-agent debate and voting mechanisms provide a practical direction to reduce such failure modes while controlling inter-agent communication costs~\cite{yang2025minimizing}.
...
\texttt{\textbf{end for}}\\
\texttt{\textbf{return} $\{\hat{\mathcal{R}}^*_u\}$}\\
\rule{\columnwidth}{1pt}
\end{algorithm}
...
\texttt{\textbf{Output:} recommendation lists $\{L_u\}$}\\
...
\texttt{\quad \textbf{for} each candidate item $i$ \textbf{do}}\\
\texttt{\quad\quad \textbf{compute} $\text{sim}(u,i)=\mathbf{p}_u\cdot\mathbf{v}_i$}\\
\texttt{\quad \textbf{end for}}\\
\texttt{\quad \textbf{rank} items by similarity to obtain $L_u$}\\
\texttt{\quad \textbf{save} $L_u$}\\
\texttt{\textbf{end for}}\\
\texttt{\textbf{return} $\{L_u\}$}\\
\rule{\columnwidth}{1pt}
\end{algorithm}
...
\texttt{\textbf{for} each query $q$ in $Q$ \textbf{do}}\\
\texttt{\quad invoke threshold+regex corrected recall (Algorithm~\ref{lst:query-recall}) to get $R_q$}\\
\texttt{\quad merge $R_q$ into Final\_R}\\
\texttt{\textbf{end for}}\\
\texttt{\textbf{return} Final\_R}\\
\rule{\columnwidth}{1pt}
\end{algorithm}
...
\texttt{\textbf{end for}}\\
\texttt{\textbf{Compute} composite scores $S(v)$ for candidates}\\
\texttt{\textbf{Sort} candidates by $S(v)$ in descending order}\\
\texttt{\textbf{return} Final\_R}\\
\rule{\columnwidth}{1pt}
\end{algorithm}
...
\begin{figure}[t]
    ...
    \IfFileExists{figures/new1.pdf}{%
        \includegraphics[width=\linewidth]{figures/new1.pdf}%
    }{%
