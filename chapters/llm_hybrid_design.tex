...
\raggedright
\texttt{\{ "Identification Info": \{}\\
\texttt{\quad "Name": <user name>,}\\
\texttt{\quad "UserID": <unique identifier>,}\\
\texttt{\quad "Age": <age>,}\\
\texttt{\quad "Gender": <gender>,}\\
\texttt{\quad "Salary": <salary>,}\\
\texttt{\quad "Federation": <organization/group>,}\\
\texttt{\quad "Work age": <years of work>,}\\
\texttt{\quad "Deposit": <deposits>,}\\
\texttt{\quad "Company": <company>,}\\
\texttt{\quad "Home Address": <home address>}\\
\texttt{\},}\\
\texttt{"Interaction Info": \{}\\
\texttt{\quad "Description of Favor": <preference description>,}\\
\texttt{\quad "Interaction History": \{}\\
\texttt{\quad\quad "Inter1": \{}\\
\texttt{\quad\quad\quad "Time": <time>,}\\
\texttt{\quad\quad\quad "Query": <query>,}\\
\texttt{\quad\quad\quad "Filter": <filters>,}\\
\texttt{\quad\quad\quad "Final decision": \{}\\
\texttt{\quad\quad\quad\quad "Item name": <item name>,}\\
\texttt{\quad\quad\quad\quad "Item category": <category>,}\\
\texttt{\quad\quad\quad\quad "Item comment grade": <rating>,}\\
\texttt{\quad\quad\quad\quad "Item price average": <avg price>,}\\
\texttt{\quad\quad\quad\quad "Item comment count": <review count>,}\\
\texttt{\quad\quad\quad\quad "Item distance": <distance>}\\
...
\rule{\columnwidth}{1pt}
\raggedright
\texttt{\{ "Query": <user query>,}\\
\texttt{\quad "Filter": \{}\\
\texttt{\quad\quad "filter type": <distance/reviews/price/rating/category>,}\\
\texttt{\quad\quad "rank type": <1: ascending, 0: descending, none>\}}\\
\texttt{\quad "Location": <user location>,}\\
\texttt{\quad "Time": <request time> \}}\\
\rule{\columnwidth}{1pt}
\end{listing}
...
\rule{\columnwidth}{1pt}
\raggedright
\texttt{Input: user set $U$, item set $I$, rating matrix $R$, rank $k$, learning rate $\eta$, regularization $\lambda$, epochs num\_epochs, attention ranges $\{K_n\}$}\\
\texttt{Output: model set $\{\hat{\mathcal{R}}^n\}$ for different attention ranges}\\
\texttt{for each $K_n$ in $\{K_n\}$ do}\\
\texttt{\quad initialize $\mathcal{U}^n \in \mathbb{R}^{m \times k}$ and $\mathcal{V}^n \in \mathbb{R}^{n \times k}$ and move to GPU}\\
\texttt{\quad for epoch = 1 to num\_epochs do}\\
\texttt{\quad\quad compute $\hat{\mathcal{R}}^n = \sum_{i=1}^N \mathcal{W}_i^n \circ (\mathcal{U}^n \Sigma \mathcal{V}^{nT})$}\\
\texttt{\quad\quad compute gradients $\partial \mathcal{L}^n/\partial \mathcal{U}^n$ and $\partial \mathcal{L}^n/\partial \mathcal{V}^n$}\\
\texttt{\quad\quad update $\mathcal{U}^n \leftarrow \mathcal{U}^n - \eta \,\partial \mathcal{L}^n/\partial \mathcal{U}^n$}\\
\texttt{\quad\quad update $\mathcal{V}^n \leftarrow \mathcal{V}^n - \eta \,\partial \mathcal{L}^n/\partial \mathcal{V}^n$}\\
\texttt{\quad end for}\\
\texttt{\quad save $\hat{\mathcal{R}}^n$ from GPU to CPU}\\
\texttt{end for and return $\{\hat{\mathcal{R}}^n\}$}\\
\rule{\columnwidth}{1pt}
\end{listing}
...
Based on Listing~\ref{lst:gpu-weighted-mf} and the quantitative shift estimation approach above, we propose the following LLM-based adaptive long--short-term recommendation procedure.
...
\rule{\columnwidth}{1pt}
\raggedright
\texttt{Input: user profile set $\{P_u\}$, model set $\{\hat{\mathcal{R}}^n\}$}\\
\texttt{Output: scheduled model $\{\hat{\mathcal{R}}^*_u\}$}\\
\texttt{for each user profile $P_u$ do}\\
\texttt{\quad Sew Agent analyzes interest change and computes $\Delta I_u$}\\
\texttt{\quad if $\Delta I_u$ exceeds a threshold then}\\
\texttt{\quad\quad select $\hat{\mathcal{R}}^*_u = \arg\max_{n}\,\text{Fitness}(K_n,\Delta I_u)$}\\
\texttt{\quad else keep the current model}\\
\texttt{\quad save $\hat{\mathcal{R}}^*_u$}\\
\texttt{end for and return $\{\hat{\mathcal{R}}^*_u\}$}\\
\rule{\columnwidth}{1pt}
\end{listing}
...
\rule{\columnwidth}{1pt}
\raggedright
\texttt{Input: users $\{u\}$, items $\{i\}$, top-$N$, vectorizer (TF--IDF/BERT/spaCy)}\\
\texttt{Output: recommendation list}\\
\texttt{for each item $i$ do}\\
\texttt{\quad if TF--IDF then compute TF--IDF vector $\mathbf{v}_i$}\\
\texttt{\quad else if BERT then preprocess $\text{Doc}_i$ and compute $\mathbf{v}_i=\text{BERT}(\text{Doc}_i)$}\\
\texttt{\quad else if spaCy then preprocess, parse, NER, and compute embedding vector $\mathbf{v}_i$}\\
\texttt{end for}\\
\texttt{for each user $u$ do}\\
\texttt{\quad compute preference vector $\mathbf{p}_u = \frac{1}{|\mathcal{I}_u|}\sum_{i\in\mathcal{I}_u}\mathbf{v}_i$}\\
\texttt{\quad for each candidate item $i$ do compute $\text{sim}(u,i)=\mathbf{p}_u\cdot\mathbf{v}_i$}\\
\texttt{\quad rank items by similarity and return top-$N$}\\
\texttt{end for}\\
\rule{\columnwidth}{1pt}
\end{listing}
...
\rule{\columnwidth}{1pt}
\raggedright
\texttt{Input: query set $\{Q_i\}$, item database, threshold $\theta$}\\
\texttt{Output: recalled item list Final\_R}\\
\texttt{Initialize Final\_R = []}\\
\texttt{for each query $Q_i$ do}\\
\texttt{\quad vectorize $Q_i$ to $\mathbf{v}_{Q_i}$; Temp\_R = []}\\
\texttt{\quad for each item $j$ in database do}\\
\texttt{\quad\quad vectorize item $j$ to $\mathbf{v}_j$ and compute sim}\\
\texttt{\quad\quad if sim($\mathbf{v}_{Q_i},\mathbf{v}_j$) $> \theta$ then add $j$ to Temp\_R}\\
\texttt{\quad end for}\\
\texttt{\quad if Temp\_R not empty then sort Temp\_R by sim desc and merge into Final\_R}\\
\texttt{end for}\\
\texttt{if Final\_R empty then perform regex matching, sort matches, and insert into Final\_R}\\
\texttt{return Final\_R}\\
\rule{\columnwidth}{1pt}
\end{listing}
...
3) \textbf{Retrieval using Listing~\ref{lst:query-recall}.}\; For each \( q_i \), retrieve similar items:
\begin{equation}
...
\rule{\columnwidth}{1pt}
\raggedright
\texttt{Input: interaction history $H$, item database, threshold $\theta$}\\
\texttt{Output: recalled item list Final\_R}\\
\texttt{Initialize Final\_R = []}\\
\texttt{Use pretrained LLM to produce query set $Q=\{q_1,\ldots,q_n\}$}\\
\texttt{for each query $q$ in $Q$ do invoke threshold+regex corrected recall (Listing~\ref{lst:query-recall})}\\
\texttt{return Final\_R}\\
\rule{\columnwidth}{1pt}
\end{listing}
...
\textbf{Behavior-based correction recall.}\; For each coarse-recalled item \( r\in R_{q_i} \), we use the attention-range-controlled model (Listing~\ref{lst:gpu-weighted-mf}) to perform further recall and then fuse results. Let \( R=\bigcup_{i=1}^n R_{q_i} \). For each \( r\in R \), we obtain:
\[
...
\raggedright
\texttt{Input: interaction history $H$, item database, threshold $\theta$}\\
\texttt{Output: final ranked list}\\
\texttt{Initialize Final\_R = []}\\
\texttt{Use pretrained LLM to generate queries $Q=\{q_1,\ldots,q_n\}$}\\
\texttt{for each query $q$ do}\\
\texttt{\quad $R_q=\text{FindSimilarItems}(q,\theta,\text{top\_n})$; Intermediate\_R=[]}\\
\texttt{\quad for each item $r$ in $R_q$ do}\\
\texttt{\quad\quad $V_r=\hat{\mathcal{R}}_n(r)$; add $V_r$ to Intermediate\_R}\\
\texttt{\quad end for; merge Intermediate\_R into Final\_R}\\
\texttt{end for}\\
\texttt{Compute composite scores $S(v)$ for candidates and sort to return final list}\\
\rule{\columnwidth}{1pt}
\end{listing}
...
Finally, given both the behavior-based correction model and the content-based recall results, we apply the fusion framework in Listing~\ref{lst:hybrid-align} to correct content recall using behavioral signals. The final ranked list thus jointly reflects both item content semantics and user interaction behaviors, and is returned for evaluation.
