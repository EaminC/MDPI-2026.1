...
% Code blocks (for readable JSON-like examples)
\usepackage{listings}
\lstdefinestyle{jsonlike}{
  ...
  xrightmargin=0.5em,
  literate=
    *{"type"}{{\bfseries "type"}}1
     {"classification"}{{\bfseries "classification"}}1
     {"name"}{{\bfseries "name"}}1
     {"filter"}{{\bfseries "filter"}}1
     {"description"}{{\bfseries "description"}}1
     {"preferred_class"}{{\bfseries "preferred\_class"}}1
     {"modified_filter"}{{\bfseries "modified\_filter"}}1
     {"item_list"}{{\bfseries "item\_list"}}1
}
...
\Author{Yiming Cheng $^{1,}$*, Hao Feng $^{2}$, Yitong Ma $^{3}$, Qi Yang $^{1}$, Yudong Zhang $^{2}$, Yi Yang $^{2,}$* and Zhu Han $^{4,}$*}

%\longauthorlist{yes}

% MDPI internal command: Authors, for metadata in PDF
\AuthorNames{Yiming Cheng, Hao Feng, Yitong Ma, Qi Yang, Yudong Zhang, Yi Yang and Zhu Han}

% Affiliations / Addresses (Add [1] after \address if there is only one affiliation.)
\address{%
$^{1}$ \quad University of Chicago, Chicago, IL, USA; eaminchan@uchicago.edu (Y.C.); yangqi@uchicago.edu (Q.Y.)\\
$^{2}$ \quad Tsinghua University, Beijing, China; yangyy@tsinghua.edu.cn (Y.Y.); zhangyd16@mails.tsinghua.edu.cn (Y.Z.); fh20@mails.tsinghua.edu.cn (H.F.)\\
$^{3}$ \quad University of California, San Diego, La Jolla, CA, USA; yim030@ucsd.edu (Y.M.)\\
$^{4}$ \quad Department of Electrical and Computer Engineering, University of Houston, Houston, TX 77004, USA; zhan2@central.uh.edu (Z.H.)}

% Contact information of the corresponding author
\corres{Correspondence: eaminchan@uchicago.edu (Y.C.); yangyy@tsinghua.edu.cn (Y.Y.); zhan2@central.uh.edu (Z.H.)}
